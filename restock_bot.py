...
def check_target_store_stock(tcin, store_id):
    ...
    proxies = {
        "http": f"http://{os.getenv('PROXY_USER')}:{os.getenv('PROXY_PASS')}@gate.smartproxy.com:10000",
        "https": f"http://{os.getenv('PROXY_USER')}:{os.getenv('PROXY_PASS')}@gate.smartproxy.com:10000",
    }
    try:
